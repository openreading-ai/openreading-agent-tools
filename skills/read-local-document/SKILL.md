...
2. Use `openreading_get_document` when available to obtain the complete retained normalized result without raw provider payloads. Its import receipt's legacy search suggestion is not a mandatory step. Search remains optional for a focused question; historical runtimes without the full-document tool use search and read.
